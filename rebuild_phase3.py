    ...
    <div className="p-8 max-w-7xl mx-auto flex flex-col gap-8 pb-20">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 max-w-3xl">Interactive hydrodynamic simulation and emergency-response platform. This system utilizes a physics-based coupled DualSPHysics and LISFLOOD-FP solver chain for prototype exposure analysis.</p>
        </div>
        <div className="flex items-center gap-3">
           <button onClick={() => onNavigate('scenarios')} className="px-5 py-2.5 bg-white border border-slate-200 text-slate-700 font-bold text-sm rounded-lg hover:bg-slate-50 transition">Compare Scenarios</button>
           <button onClick={() => onNavigate('simulation')} className="px-5 py-2.5 bg-blue-600 text-white font-bold text-sm rounded-lg hover:bg-blue-700 transition flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-slate-100 rounded-2xl border border-slate-200 h-[400px] flex items-center justify-center relative overflow-hidden">
           <img src="https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/13/3389/5801" alt="Tehri Study Area" className="absolute inset-0 w-full h-full object-cover opacity-60 mix-blend-luminosity" />
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-5 flex-1 flex flex-col justify-between">
             ...
             <button onClick={() => onNavigate('hadr')} className="mt-6 w-full py-2 bg-slate-800 hover:bg-slate-900 text-white text-sm font-bold rounded-lg transition">View HADR Operations</button>
          </div>
        </div>
      </div>
        ...
        <div className="flex items-center justify-between gap-2 overflow-x-auto pb-4">
          {PIPELINE.map((step, i) => (
             <React.Fragment key={step.label}>
               <div className="flex-1 min-w-[120px] bg-slate-50 border border-slate-200 rounded-lg p-3 text-center flex flex-col items-center justify-center relative">
