# FloodLab — Tehri Reservoir Inflow Hydrograph Report (Step 7B Validation)
...
- **Conceptual Structure**: Infiltration loss separation ($f_{\text{loss}} = 0.05\text{ mm/hr}$) + Nash Cascade linear quickflow routing ($N=1.875, K=10.75\text{ hr}$) + Linear baseflow/melt reservoir ($K=38.1\text{ hr}, Q_0=200.0\text{ m}^3/\text{s}$).
...
- **Validation Metrics**:
  - Event Window (144h): NSE = **-0.4436**, KGE = **0.3096**, $r$ = **0.3179**, RMSE = **148.18 m³/s**, MAE = **123.99 m³/s**, Peak Error = **+1.63%**, Timing Error = **+5h**, Volume Bias = **+8.58%**.
  - Full Simulation (264h): NSE = **-1.2102**, KGE = **0.1032**, $r$ = **0.1254**, RMSE = **157.57 m³/s**, MAE = **131.93 m³/s**, Volume Bias = **-5.39%**.

## 3. Water-Balance Component Separation (Event Window: 2024-07-30 to 2024-08-04)
...
| :--- | :---: | :---: | :---: | :---: | :---: | :---: |
| **Upper Bhagirathi (Tekhla)** | 300.05 | 199.35 | 125.91 | 325.26 | **0.664** (66.4%) | 1.084 |
...
## 4. Total Tehri Reservoir Inflow Hydrograph ($Q_{\text{TEHRI\_IN}}(t)$)
...
## 5. Downstream Observation Status
